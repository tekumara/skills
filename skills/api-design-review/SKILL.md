...
description: Reviews an API's endpoint design by inventorying routes, finding overlapping endpoint scope, and detecting inconsistent success-response payloads. Use when asked to review API routes, compare endpoints, find redundant or superseding operations, assess pagination contract consistency, or identify API design issues across REST endpoints.
---
...
1. Establish the target version or revision. If the working copy and default branch differ, state which one is reviewed.
...
7. Verify each finding against implementation evidence. Do not report a difference as an inconsistency when it follows from a materially different operation and does not burden the same consumers.
...
- Do not redesign unrelated naming, authentication, errors, or resources unless the user asks for a broader review.
...
1. **Issue** — overlap classification or payload inconsistency.
