...
- Required outputs: endpoint inventory, a difference table for each overlapping endpoint group, an optional auth column when auth differs, one priority-tagged consolidated finding list covering rubric and non-rubric issues, a REST rule coverage table that references findings without repeating them, evidence-backed recommendations, and excluded deprecated routes
